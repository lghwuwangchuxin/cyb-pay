...
public class HMacMD5 {
	...
	public static boolean checkSign(String str, String key) throws Exception{
		if (str == null || key == null) {
			return false;
		}
		//接收到的json str按照顺序转换为JSONObject
		LinkedHashMap<String, Object> json = JSON.parseObject(str,LinkedHashMap.class, Feature.OrderedField);
		JSONObject jsonObject=new JSONObject(true);
		jsonObject.putAll(json);
		
		String signature = jsonObject.getString("signature");//获取签名
		jsonObject.remove("signature");//删除签名
		String signature2 = encodeMac(jsonObject.toString(),key);//生成签名验证
