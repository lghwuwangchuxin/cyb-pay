...
public class PostUtil {
	...
	public static String executesss(String reqMsg,String url) {
        ...
        try {
            ...
            //String sign = "no_sign";
            //System.out.println("待签名数据："+ SignUtils.getSignData(jsonParam));
             //sign = "no_sign";
             /**
            ...
            System.out.println("设置https，绕过证书。");
            ...
            httpsUrlConnection.setRequestProperty("Content-Length", String.valueOf(reqMsg.getBytes().length));
            httpsUrlConnection.setDoOutput(true);
            httpsUrlConnection.setDoInput(true);
            System.out.println("设置https连接参数。");
            //logger.info("设置https连接参数。");
            GZIPOutputStream gzip;  
            try {  
                gzip = new GZIPOutputStream(httpsUrlConnection.getOutputStream());  
                gzip.write(reqMsg.getBytes("utf-8")); 
                gzip.flush();
                gzip.close();  
            } catch (IOException e) {  
                logger.info("gzip compress error.", e);
            ...
            brr = new BufferedReader(new InputStreamReader(httpsUrlConnection.getInputStream()));
            resultStr = brr.readLine();
            System.out.println("应答报文:" + resultStr);
